...
export function registerAllTools(server) {
    ...
    server.tool('list-components', 'List all available IGDS components for a specific framework', {
        framework: z.enum(['angular', 'react', 'core-web']).describe('Framework to list components for'),
    }, async ({ framework }) => {
        const components = useCache
            ? cachedLoader.listComponents(framework)
            : liveStore.listComponents(framework);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    // Tool: Get component
    server.tool('get-component', 'Get detailed documentation for a specific IGDS component', {
        ...
        const component = useCache
            ? cachedLoader.getComponent(framework, componentName)
            : liveStore.getComponent(framework, componentName);
        if (!component) {
            return {
                content: [{
                        type: 'text',
    ...
    server.tool('get-component-source', 'Get source code structure for a component (properties, CSS, defaults). Requires cached data.', {
        framework: z.enum(['angular', 'react', 'core-web']).describe('Framework'),
        componentName: z.string().describe('Component name'),
    }, async ({ framework, componentName }) => {
        if (!useCache) {
        ...
        const source = cachedLoader.getSource(framework, componentName);
        if (!source) {
            return {
                content: [{
                        type: 'text',
    ...
    // Tool: Get story
    server.tool('get-story', 'Get a specific story entry by its ID', {
        framework: z.enum(['angular', 'react', 'core-web']).describe('Framework'),
        storyId: z.string().describe('Story ID'),
    }, async ({ framework, storyId }) => {
        const entry = useCache
            ? cachedLoader.getStoryEntry(framework, storyId)
            : liveStore.getStoryEntry(framework, storyId);
        if (!entry) {
            return {
                content: [{
                        type: 'text',
    ...
    server.tool('get-component-stories', 'Get all story variants for a component', {
        ...
        const stories = useCache
            ? cachedLoader.getStories(framework, componentName)
            : component.stories;
        return {
            content: [{
                    type: 'text',
    ...
    server.tool('get-component-css', 'Get CSS styles for a component. Requires cached data.', {
        framework: z.enum(['angular', 'react', 'core-web']).describe('Framework'),
        componentName: z.string().describe('Component name'),
    }, async ({ framework, componentName }) => {
        if (!useCache) {
