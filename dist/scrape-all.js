...
async function fetchIndex(framework) {
...
function getComponentsFromIndex(index) {
            ...
            for (const row of rows) {
                ...
                if (cells.length >= 4) {
                    ...
                    if (radios && radios.length > 0) {
                        control = 'select';
                        enumValues = Array.from(radios).map((r) => r.value || r.nextElementSibling?.textContent?.trim() || '');
                    }
    ...
    for (const framework of frameworks) {
        ...
        if (existingDocsCount < components.size) {
            ...
            for (const [componentName, componentId] of components) {
                if (data[framework][componentName])
                    continue;
                count++;
        ...
        if (existingStories < totalComponents) {
            ...
            for (const [componentName, docs] of Object.entries(data[framework])) {
                ...
                if (data.storyExamples[framework][componentName])
                    continue;
                // Get story IDs from index
                const index = data.indexes[framework];
                const storyEntries = Object.values(index.entries).filter(e => e.type === 'story' && e.title.includes(componentName));
