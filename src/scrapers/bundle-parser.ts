...
async function fetchReactChunks(entryCode: string, assetsBase: string): Promise<string> {
  ...
  const chunkCodes: string[] = [entryCode];
  let fetched = 0;
...
function extractFromDocgenInfo(bundleCode: string): Map<string, ComponentSource> {
  const components = new Map<string, ComponentSource>();
  ...
  while (true) {
    ...
    if (propsIdx !== -1) {
      ...
      while (propsPos < docgenStr.length && propsDepth > 0) {
        if (docgenStr[propsPos] === '{') propsDepth++;
        if (docgenStr[propsPos] === '}') propsDepth--;
        propsPos++;
      }
