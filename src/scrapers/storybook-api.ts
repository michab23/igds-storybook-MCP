 ...
 */
export function parseLiteralUnion(typeText: string | undefined): string[] | undefined {
  if (!typeText?.includes('|')) return undefined;

  const parts = typeText.split('|').map((part) => part.trim());
  const literalParts = parts.filter(
    (part) => !TYPE_KEYWORDS.has(part.replace(/^['"]|['"]$/g, '').toLowerCase())
  );
  if (literalParts.length < 1) return undefined;
  if (!literalParts.every((part) => /^(['"]).*\1$/.test(part))) return undefined;
 ...
 */
export async function extractArgTypesFromPreview(page: Page, storyId: string): Promise<ArgType[]> {
  const raw = await page.evaluate(async (id: string) => {
    ...
    try {
      ...
      if (!story?.argTypes) return null;

      // Structured-clone safety: keep only the fields we consume.
      return Object.entries(story.argTypes).map(([name, value]: [string, any]) => ({
        ...
        defaultValue:
          value?.table?.defaultValue?.summary !== undefined
 ...
 */
function normalizeArgType(entry: RawArgType): ArgType {
 ...
 */
export async function extractStorySources(
  page: Page,
  baseUrl: string,
  ...
  try {
    ...
    if (count) await page.waitForTimeout(2000);
  } catch {
...
interface RawDocumentedGroup {
  component: string;
  properties: { name: string; type: string; required: string; description: string }[];
  events: { name: string; payload: string }[];
}

export async function extractDocumentedApi(page: Page): Promise<DocumentedComponentApi[]> {
