...
server.tool(
  'list-components',
  'List all available IGDS components for a framework',
  {
    framework: z.enum(['angular', 'react', 'core-web']).describe('Framework to list components for'),
  },
  async ({ framework }) => {
    const fw = framework as StorybookFramework;
...
server.tool(
  'get-component',
  'Get detailed documentation for a specific IGDS component including argTypes and source code',
  {
...
server.tool(
  ...
  'Get the source code structure for a component (properties, CSS, defaults)',
  {
    framework: z.enum(['angular', 'react', 'core-web']).describe('Framework'),
    componentName: z.string().describe('Component name (e.g., "Accordion", "Button")'),
  },
...
server.tool(
  'get-story',
  'Get story entry details by its ID',
  {
    framework: z.enum(['angular', 'react', 'core-web']).describe('Framework'),
...
server.tool(
  ...
  async ({ framework, componentName }) => {
    ...
    const componentEntries = Object.values(entries).filter(e => 
      e.title.toLowerCase().includes(componentName.toLowerCase())
    );
