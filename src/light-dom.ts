 ...
 */
export function canonicalAttributeName(name: string): string {
  return name
    .replace(/^[[(*@]+/, '')
    .replace(/[\])]+$/, '')
 ...
 */
export function mineAttributes(html: string, into: MinedTags = new Map()): MinedTags {
  if (!html) return into;
  ...
  while ((match = TAG_PATTERN.exec(html)) !== null) {
    ...
    if (isClosing || !tagName.startsWith('igds-')) continue;
    ...
    for (const { name, value } of parseAttributes(match[3] || '')) {
      ...
      if (/^(undefined|null)$/i.test(decoded)) continue;
      if (decoded.length > MAX_MINED_VALUE_LENGTH) continue;
      if (!mined.values.includes(decoded)) mined.values.push(decoded);
    }
  }
 ...
 */
export function mergeSquashedAttributes(mined: MinedTags): MinedTags {
  for (const [tag, attributes] of mined) {
    ...
    for (const names of byKey.values()) {
      ...
      const canonical = names.reduce((best, name) =>
        (name.match(/-/g)?.length || 0) > (best.match(/-/g)?.length || 0) ? name : best
      );
 ...
 */
export function looksLikeEnum(values: string[]): boolean {
...
function cleanAttributes(raw: string): string {
  ...
  for (const { name, value } of parseAttributes(raw)) {
    ...
    if (canonical !== 'dir' && isNoiseAttribute(name)) continue;
    if (!/^[a-z][\w-]*$/.test(canonical)) continue;
 ...
 */
export function toUsageSnippet(html: string, maxElements = 6): string | undefined {
  ...
  while ((match = TAG_PATTERN.exec(withoutStyles)) !== null) {
    ...
    if (snippets.length >= maxElements) break;
  }
  ...
  const cleaned = snippets
    .slice(0, maxElements)
    .map((snippet) =>
      snippet.replace(TAG_PATTERN, (_full, closing: string, tag: string, attrs: string, selfClose: string) => {
...
function dedupe(snippets: string[]): string[] {
